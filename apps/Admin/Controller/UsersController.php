...

{
...

    {
...

        {
            ...
            {
                $new->setPassword($user->getPassword(), false);
            }
...

    {
...

        {
            ...
            return $this->redirect($this->generateUrl('users_search', array(
                ...
                'last_login' => ($request->request->get('last_login') === '' ?  null : $request->request->get('last_login')),
        ...
        //www.mkapuscinski.local/backend.php/users/search?last_login=-1+year
        ...
        $lastLogin = '';
        if($request->query->has('last_login'))

        {

            $lastLogin = 'last_login < "' . date('Y-m-d H:i:s', strtotime($request->query->get('last_login'))) . '"';

        }



        if(empty($query) && empty($lastLogin))
        ...
        $list = $this->users->findUsers($query, $lastLogin);
        ...
        $pagination->
            ...
            setBaseUrl($this->generateUrl('users_search', array(
                ...
                'last_login' => $request->query->get('last_login'),
