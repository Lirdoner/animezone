...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-tasks"></i> Lista serwerów <span class="badge"><?= $total ?></span></h3>
        <a href="<?= $app->generateUrl('servers_create') ?>" class="btn btn-xs btn-success pull-right btn-helper"><i class="fa fa-plus-square"></i>Utwórz nowy</a>
    </div>
    <?php if($list->rowCount()): ?>
        <form method="post" action="<?= $app->generateUrl('servers_update') ?>">
            <table class="table table-bordered table-striped table-hover" style="margin-bottom: 0">
                <thead>
                <tr>
                    <th colspan="2">Serwer</th>
                ...
                <?php foreach($list->fetchAll() as $row): ?>
                    <tr>
                        ...
                        <td class="col-sm-10">
                            <a href="<?= $app->generateUrl('servers_edit', array('serverID' => $row['id'])) ?>"><?= $row['name'] ?></a>
                        </td>
                        <td class="text-center"><?= $row['mobile'] ? 'Tak' : 'Nie' ?></td>
                        <td class="text-center" style="padding: 8px 0 0 0">
                            <div class="btn-group btn-group-xs">
                                ...
                                <ul class="dropdown-menu text-left" role="menu">
                                    <li><a href="<?= $app->generateUrl('servers_edit', array('serverID' => $row['id'])) ?>"><i class="fa fa-edit"></i> Edytuj</a></li>
                                    <li><a href="<?= $app->generateUrl('servers_delete', array('serverID' => $row['id'])) ?>"><i class="fa fa-times"></i> Usuń</a></li>
                                </ul>
                            </div>
                        </td>
                    </tr>
