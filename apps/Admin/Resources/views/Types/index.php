...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3 class="panel-title"><i class="glyphicon glyphicon-bookmark"></i> Lista Typów <span class="badge"><?= $total ?></span></h3>
        <a href="<?= $app->generateUrl('types_create') ?>" class="btn btn-xs btn-success pull-right btn-helper"><i class="fa fa-plus-square"></i>Utwórz nowy</a>
    </div>
    <?php if($list->rowCount()): ?>
        <form method="post" action="<?= $app->generateUrl('types_update') ?>">
            <table class="table table-bordered table-striped table-hover" style="margin-bottom: 0">
                ...
                <?php foreach($list->fetchAll() as $row): ?>
                    <tr>
                        ...
                        <td class="col-sm-11"><a href="<?= $app->generateUrl('types_edit', array('typeID' => $row['id'])) ?>"><?= htmlspecialchars($row['name']) ?></a></td>
                        <td class="text-center" style="padding: 8px 0 0 0">
                            <div class="btn-group btn-group-xs">
                                ...
                                <ul class="dropdown-menu text-left" role="menu">
                                    <li><a href="<?= $app->generateUrl('types_edit', array('typeID' => $row['id'])) ?>"><i class="fa fa-edit"></i> Edytuj</a></li>
                                    <li><a href="<?= $app->generateUrl('types_delete', array('typeID' => $row['id'])) ?>"><i class="fa fa-times"></i> Usuń</a></li>
                                </ul>
                            </div>
                        </td>
                    </tr>
